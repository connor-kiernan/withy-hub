...
const EditTrainingForm = ({training}) => {
  ...
  const dateObj = new Date(training.kickOffDateTime);
  ...
  const [address1, setAddress1] = useState(training.address.line1);
  const [address2, setAddress2] = useState(training.address.line2 ?? "");
  const [postcode, setPostcode] = useState(training.address.postcode);
  const [pitchType, setPitchType] = useState(training.pitchType);
  ...
  return (
      <>
        <Form noValidate onSubmit={submitForm} validated={validated} id="editTrainingForm">
          <legend>Edit Training Session</legend>
          <Row className="mb-4">
        ...
        <Toast onClose={() => setShowSuccess(false)} show={showSuccess} delay={3000} autohide
          ...
          <div className="d-flex">
            <Toast.Body>Training session updated successfully</Toast.Body>
        ...
        <Toast onClose={() => setShowError(false)} show={showError} delay={6000} autohide
               className="position-fixed bottom-0 start-50 translate-middle-x mb-6">
          <div className="d-flex">
            <Toast.Body>Error updating training session: {error?.data?.title}, please try again</Toast.Body>
            <CloseButton className="me-2 m-auto" data-bs-dismiss="toast" aria-label="close"/>
          </div>
        </Toast>
      </>
